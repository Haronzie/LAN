    ...
    try {
      if (moveItem.type === 'directory') {
        ...
        try {
          ...
          const checkUrl = `${BASE_URL}/files?directory=${encodeURIComponent(moveDestination || '')}`;
          const checkRes = await axios.get(checkUrl, { withCredentials: true });
          
          const fileExists = (checkRes.data || []).some(
            f => f.name === moveItem.name && (!f.directory || f.directory === moveDestination)
          );
          ...
          if (fileExists && !overwrite) {
            ...
            return new Promise((resolve) => {
              FileOperationConflictModal({
                ...
                onKeepBoth: async () => {
                  try {
                    // Let the server handle the renaming by not setting overwrite
                    await axios.post(
                      `${BASE_URL}/move-file`,
                      {
                        id: fileId,
